...
import { glueName, normalizePrefix, parseParquetKey } from './partitions.js';
import { downloadObjectAsArrayBuffer, listAllObjects } from './s3.js';
...
async function main() {
  ...
  // Runs through all datasets and creates or updates the corresponding Glue tables and partitions
  for (const [datasetName, dataset] of datasets) {
    ...
    const buffer = await downloadObjectAsArrayBuffer(s3, bucket, dataset.files[0]);
    const columns = await readParquetSchema(buffer);
    console.log(`Columns: ${columns.map((column) => `${column.Name}:${column.Type}`).join(', ')}`);

    const partitionNames = dataset.partitionNames ?? [];
    const tableLocation = `s3://${bucket}/${prefix}${datasetName}/`;
    const result = await saveTable(glue, database, tableName, tableLocation, columns, partitionNames);
    ...
    const created = await savePartitions(glue, database, tableName, columns, dataset.partitions);
    console.log(`Partitions: ${created} created, ${dataset.partitions.length - created} already present.`);
  }
}
...
function groupFilesByDataset(objects) {
  ...
  for (const object of objects) {
    ...
    if (!datasets.has(parsed.dataset)) {
      datasets.set(parsed.dataset, { files: [], partitions: [], partitionNames: null });
    }
    ...
    if (dataset.partitionNames === null) dataset.partitionNames = names;
    if (JSON.stringify(names) !== JSON.stringify(dataset.partitionNames)) {
      console.warn(`Skipping incompatible partition path: ${object.Key}`);
      continue;
    }

    const values = parsed.partitions.map((item) => item.value);
    const alreadyFound = dataset.partitions.some((item) => JSON.stringify(item.values) === JSON.stringify(values));
    if (!alreadyFound) {
      dataset.partitions.push({ values, location: `s3://${bucket}/${parsed.partitionLocation}` });
    }
  }
  return datasets;
}
